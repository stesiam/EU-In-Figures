...
# Function to create a custom gradient background
gradient_grob <- function(width, height, color1, color2) {
  # Create a sequence of colors from color1 to color2
  gradient_colors <- colorRampPalette(c(color1, color2))(100)
  
  # Create a background grob with the gradient
  rectGrob(
    gp = gpar(fill = gradient_colors, col = NA),
    width = unit(width, "cm"),
    height = unit(height, "cm"), just = "right"
  )
}
...
# Get the width and height of the title text
text_width <- convertWidth(grobWidth(title_grob_text), "cm")
text_height <- convertHeight(grobHeight(title_grob_text), "cm")

# Create the gradient background
gradient <- gradient_grob(1.3*text_width, 1.2*text_height, "#FF7F50", "#8A2BE2")
# Combine the gradient background and the text
final_grob <- grobTree(gradient, title_grob_text)

# Example: Define your map_plot
# Let's assume map_plot is a ggplot object that you've already created

# Adjust the annotation size relative to map coordinates
final_plot <- map +
  annotation_custom(final_grob, 
                    xmin = 2700000, xmax = 5800000, 
                    ymin = 5200000, ymax = 5350000)
